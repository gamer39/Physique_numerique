...
alpha = 0.54*1e-6*facteur_dimension**2 #béton: 0.54 m²/s, laine de verre: 0.58 m²/s, liège: 0.115 m²/s
delta_x = 1
delta_t = (delta_x**2)/(6*alpha)    #Pour la stabilité, delta_t <= delta**2/2*alpha
y_plan = int(longueur/2)
cap_béton = 2500 #kJ/m³K
...
if val_cart == "Oui" or val_cart == "OUI" or val_cart == "oui":
    ...
    # Show Figure
    anim.save(f'/home/alicecalice/Documents/Physique numérique/git2/Conduction_3D_et_2D_{var_saison}.gif', fps=2)

    plt.show()

    q_beton = transfert_thermique_cart(T, largeur_mur, largeur, longueur, hauteur, facteur_dimension, cap_béton, delta_t, temps_iter-1, 0)
...
if val_cyl == "Oui" or val_cyl == "OUI" or val_cyl == "oui":
    ...
    #     # # Show Figure
    anim.save(f'/home/alicecalice/Documents/Physique numérique/git2/Conduction_cylindrique_{var_saison}.gif', fps=2)

    plt.show()

    q_beton_cyl = transfert_thermique_cyl(T_cyl, largeur_mur, 2*longueur_r, hauteur, facteur_dimension, cap_béton, delta_t, temps_iter-1, 0)
    print('transfert thermique BETON CYLINDRIQUE', q_beton_cyl, 'kW')
